    ...
    <div className="app-shell">
      <header className="topbar">
        <Link className="brand" href={canManage ? "/admin" : "/user"}>
          ...
          <span><strong>资源管理系统</strong><small>{canManage ? "管理端" : "用户端"}</small></span>
        </Link>
        <nav className="nav" aria-label="主导航">
          {links.map((link) => (
            <Link className={pathname === link.href ? "active" : ""} href={link.href} key={link.href}>
              {link.label}
              {"count" in link && link.count ? <span className="nav-count">{link.count}</span> : null}
            </Link>
          ))}
        </nav>
        <div className="topbar-actions">
          <Link className="mobile-entry-pill" href="/mobile">现场端</Link>
          <form action={logoutAction}><button className="logout-button" type="submit">退出</button></form>
        </div>
      </header>
